...
| Ressource             | Profil référencé              | Service responsable    |
|-----------------------|-------------------------------|------------------------|
| `Patient`             | profil Bureau des entrées     | Bureau des entrées     |
| `Soignant`            | profil RH (Practitioner)      | Ressources Humaines    |
| `SoignantRole`        | profil RH (PractitionerRole)  | Ressources Humaines    |
...
- **RH** : on référence directement leurs profils `Soignant` et `SoignantRole` dans `participant[practitioner].actor`. Le système RPPS et la cardinalité 1..1 sont alignés.
- **Bureau des entrées** : on référence leur profil `Patient` dans `participant[patient].actor`.
- **Patrimoine** : on référence leur profil `Location` dans `participant[location].actor`.
