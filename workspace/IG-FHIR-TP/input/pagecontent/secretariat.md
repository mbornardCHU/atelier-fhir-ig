...
| `PractitionerRole`| Ressources Humaines    |
| `Location`        | Gestion du patrimoine  |

> On référence les **types de base FHIR** (`Practitioner` / `PractitionerRole`), pas un profil spécifique. Cela permet d'accepter le travail des **deux équipes RH**, qu'elles aient nommé leur profil `Soignant`, `Practitioner` ou autre — toute ressource dont le `resourceType` est `Practitioner` ou `PractitionerRole` est acceptée.
...
- **RH (les deux équipes)** : on référence les types de base `Practitioner` et `PractitionerRole`, ce qui accepte le travail des deux équipes peu importe leur nommage de profil. Le système RPPS et la cardinalité 1..1 sont alignés.
